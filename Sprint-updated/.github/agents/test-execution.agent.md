...
Before starting test execution, confirm or prompt the user for execution settings:

1. **Headless vs. Headed Mode**:
   - `Headed Mode`: `HEADLESS=false` (Runs with visible browser windows for inspection)
   - `Headless Mode`: `HEADLESS=true` (Runs in background for fast execution)
   
2. **Single vs. Multiple Browsers**:
   - `Single Browser`: Runs on `Chromium` (`BROWSER=chromium`)
   - `Multiple Browsers`: Runs on all 3 browsers (`Chromium`, `Firefox`, `WebKit`) via `BROWSER=chromium,firefox,webkit`
...
- Required test-data inputs (`specs/test-cases/test-data.xlsx`) are present.
...
After preflight passes and before starting Cucumber, clean the previous run's generated evidence: `allure-results/`, `allure-report/`, `test-results/cucumber-report.json`, `test-results/cucumber-report.html`, and failure screenshots. Do not mix historical results with the current run.

Run the tests using the project's **Cucumber command/configuration**:
- Single browser: `npm test` or `npm run test:headed` / `npm run test:headless`
- Multi browser: `npm run test:multi-browser`

Immediately upon completion of test execution:
- Automatically generate the Allure report: `npx allure generate allure-results -o allure-report --clean` (or `npm run report:allure`).
- Automatically trigger downstream agents (`RTM Agent` and `Test Summary Agent`).
