...
export type BrowserExecutionMode = 'single' | 'multi';
...
function readBrowserMode(): BrowserExecutionMode {
  const rawValue = readEnv('BROWSER_MODE').toLowerCase();
  if (rawValue === 'multi' || rawValue === 'all' || readEnv('BROWSERS').includes(',')) {
    return 'multi';
  }
  return 'single';
}
...
export function getRuntimeConfig(): RuntimeConfig {
  ...
  const selectedBrowsers: BrowserName[] = browserMode === 'multi' 
    ? ['chromium', 'firefox', 'webkit'] 
    : [readBrowserName()];

  return {
    baseUrl: readEnv('FREECRM_BASE_URL') || readEnv('BASE_URL') || 'https://ui.freecrm.com',
    email: readEnv('FREECRM_EMAIL'),
    password: readEnv('FREECRM_PASSWORD'),
    browserName: readBrowserName(),
    browserMode,
    browsers: selectedBrowsers,
    headless: isDebug ? false : readBoolean('HEADLESS', true)
  };
}
